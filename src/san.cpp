...
auto parse_san(const std::string &san, chesscore::Color side_to_move) -> SANMove {
    if (san == "O-O") {
        ...
        return SANMove{san, chesscore::Piece{.type = chesscore::PieceType::King, .color = side_to_move}, target_square, false, CheckState::None};
    }
    if (san == "O-O-O") {
    ...
    return SANMove{san, chesscore::Piece::WhitePawn, chesscore::Square::A1, false, CheckState::None};
}
